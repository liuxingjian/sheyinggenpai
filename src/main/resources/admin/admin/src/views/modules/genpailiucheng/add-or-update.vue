<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="操作日期" prop="caozuoriqi">
            <el-date-picker
        ...
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.caozuoriqi" label="操作日期" prop="caozuoriqi">
              <el-input v-model="ruleForm.caozuoriqi" 
                placeholder="操作日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="流程内容" prop="liuchengneirong">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="流程内容"
                  v-model="ruleForm.liuchengneirong" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.liuchengneirong" label="流程内容" prop="liuchengneirong">
                    <span>{{ruleForm.liuchengneirong}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.anpaishijian = true;
          ...
          if(o=='liuchengneirong'){
            this.ruleForm.liuchengneirong = obj[o];
	    this.ro.liuchengneirong = true;
            continue;
          }
          if(o=='gonghao'){
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `genpailiucheng/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.genpailiuchengCrossAddOrUpdateFlag = false;
						 this.parent.search();
