<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.taocantupian" label="套餐图片" prop="taocantupian">
          <file-upload
          tip="点击上传套餐图片"
          action="file/upload"
          :limit="3"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.taocantupian" label="套餐图片" prop="taocantupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.taocantupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.taocanmingcheng = true;
          ...
          if(o=='taocantupian'){
            this.ruleForm.taocantupian = obj[o];
	...
	this.ruleForm.taocanxiangqing = this.ruleForm.taocanxiangqing.replace(reg,'../../../springboot808t0/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.taocantupian!=null) {
		this.ruleForm.taocantupian = this.ruleForm.taocantupian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `sheyinggenpai/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.sheyinggenpaiCrossAddOrUpdateFlag = false;
						 this.parent.search();
	...
	this.ruleForm.taocantupian = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
